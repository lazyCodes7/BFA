...
class BFA(object):
    ...
    def flip_bit(self, m):
        ...
        b_grad_topk = w_grad_topk * vit_conv.b_w.data
        ...
        w_bin = int2bin(m.weight.detach().view(-1), vit_conv.N_bits).short()
        w_bin_topk = w_bin[w_idx_topk]  # get the weights whose grads are topk
        # generate two's complement bit-map
        b_bin_topk = (w_bin_topk.repeat(vit_conv.N_bits,1) & vit_conv.b_w.abs().repeat(1,k_top).short()) \
        // vit_conv.b_w.abs().repeat(1,k_top).short()
        ...
        w_bin_topk_flipped = (bit2flip.short() * vit_conv.b_w.abs().short()).sum(0, dtype=torch.int16) \
                ^ w_bin_topk
        ...
        param_flipped = bin2int(w_bin,
                                vit_conv.N_bits).view(m.weight.data.size()).float()
